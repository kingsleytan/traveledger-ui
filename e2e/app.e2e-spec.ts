...
describe('Starting tests for traveledger-ui', function() {
  ...
  it('website title should be traveledger-ui', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('traveledger-ui');
    })
  });
  ...
  it('navbar-brand should be traveledger-ui',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('traveledger-ui');
    });
  });
